...
<label><b>Version: </b></label><span>");
 ...
 if (spoiler.CustomJunkLocationsString != null) { 
            this.Write("\n<label><b>Enforce Junk Locations List: </b></label><span>");
 ...
 if (spoiler.GossipHints != null && spoiler.GossipHints.Any()) { 
	...
	foreach (var hint in spoiler.GossipHints.OrderBy(h => h.Key.ToString())) { 
            ...
            this.Write("\n<script>\n\tfunction all(list, predicate) {\n\t\tfor (var i = 0; i < list.length; i++" +
                    ...
                    "; i < logic.length; i++) {\n\t\t\tvar item = logic[i];\n\t\t\titem.IsAvailable = \n\t\t\t\t(i" +
                    "tem.RequiredItemIds === null || item.RequiredItemIds.length === 0 || all(item.Re" +
                    "quiredItemIds, function(id) { return logic[id].Acquired; }))\n\t\t\t\t&& \n\t\t\t\t(item.C" +
                    "onditionalItemIds === null || item.ConditionalItemIds.length === 0 || any(item.C" +
                    "onditionalItemIds, function(conditionals) { return all(conditionals, function(id" +
                    ") { return logic[id].Acquired; }); }));\n            \n\t\t\tif (!item.Acquired && it" +
                    "em.IsFakeItem && item.IsAvailable) {\n\t\t\t\titem.Acquired = true;\n\t\t\t\trecalculate =" +
                    " true;\n\t\t\t}\n\t\t\tif (item.Acquired && item.IsFakeItem && !item.IsAvailable) {\n\t\t\t\t" +
                    "item.Acquired = false;\n\t\t\t\trecalculate = true;\n\t\t\t}\n        \n\t\t\tvar locationRow " +
                    "= document.querySelector(\".item-replacements tr[data-newlocationid=\'\" + i + \"\']\"" +
                    ");\n\t\t\tif (locationRow) {\n\t\t\t\tlocationRow.className = \"\";\n\t\t\t\tlocationRow.classLi" +
                    "st.add(item.IsAvailable ? \"available\" : \"unavailable\");\n\t\t\t\tvar itemName = locat" +
                    "ionRow.querySelector(\".itemname\");\n                var checkbox = locationRow.qu" +
                    "erySelector(\"input\");\n                checkbox.checked = item.Checked;\n\t\t\t\tif (i" +
                    "tem.Checked) {\n\t\t\t\t\titemName.classList.remove(\"spoiler\");\n\t\t\t\t} else {\n\t\t\t\t\titem" +
                    "Name.classList.add(\"spoiler\");\n\t\t\t\t}\n\t\t\t}\n        \n\t\t\tvar itemRow = document.que" +
                    "rySelector(\"#item-locations tr[data-id=\'\" + i + \"\']\");\n\t\t\tif (itemRow) {\n\t\t\t\tvar" +
                    " itemName = itemRow.querySelector(\".newlocation\");\n                var checkbox " +
                    "= itemRow.querySelector(\"input\");\n                checkbox.checked = item.Acquir" +
                    "ed;\n\t\t\t\tif (item.Acquired) {\n\t\t\t\t\titemName.classList.remove(\"spoiler\");\n\t\t\t\t} el" +
            ...
            this.Write(";\n\n    for (var i = 0; i < logic.length; i++) {\n        var item = logic[i];\n    " +
                    ...
                    "r id of startingLocations) {\n\t    logic[id].Checked = true;\n\t    logic[document." +
                    "querySelector(\"tr[data-newlocationid=\'\" + id + \"\']\").dataset.id].Acquired = true" +
                    ";\n\t    document.querySelector(\"tr[data-newlocationid=\'\" + id + \"\'] input\").check" +
                    "ed = true;\n    }\n\n\trecalculateItems();\n\n\tvar rows = document.querySelectorAll(\"t" +
                    "r\");\n\tfor (var i = 1; i < rows.length; i++) {\n\t\tvar row = rows[i];\n\t\tvar checkbo" +
                    "x = row.querySelector(\"input\");\n\t\tif (checkbox) {\n\t\t\tcheckbox.addEventListener(\"" +
                    "click\", function(e) {\n\t\t\t\tvar row = e.target.closest(\"tr\");\n                var " +
                    "rowId = parseInt(row.dataset.id);\n\t\t\t\tvar newLocationId = parseInt(row.dataset.n" +
                    "ewlocationid);\n\t\t\t\tlogic[newLocationId].Checked = e.target.checked;\n            " +
                    "    logic[rowId].Acquired = e.target.checked;\n\t\t\t\trecalculateItems();\n\t\t\t});\n\t\t}" +
                    "\n\t}\n\n\tdocument.querySelector(\"#highlight-checks\").addEventListener(\"click\", func" +
                    "tion(e) {\n        var tables = document.querySelectorAll(\"table.item-replacement" +

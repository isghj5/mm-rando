    ...
    public class Enemies
    {
        ...
        public static void ShuffleObjects(SceneEnemizerData thisSceneData)
        {
            ...
            for (int objectIndex = 0; objectIndex < thisSceneData.Objects.Count; objectIndex++)
            {
                ...
                //if (TestHardSetObject(GameObjects.Scene.TerminaField, GameObjects.Actor.Leever, GameObjects.Actor.NorthTFIceBlock)) continue;
        ...
        public static void ShuffleEnemies(OutputSettings outputSettings, CosmeticSettings cosmeticSettings, Models.RandomizedResult randomized, int randomizedSeed)
        {
            try
            {
                ...
                using (StreamWriter sw = new StreamWriter(_outputSettings.OutputROMFilename + "_EnemizerLog.txt", append: true))
                {
                    ...
                    sw.Write("Enemizer version: Isghj's Enemizer Test 61.0\n");
                    sw.Write("seed: [ " + seed + " ]");
                }
            }
            catch (Exception e)

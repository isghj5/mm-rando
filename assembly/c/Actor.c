...
void Actor_HyperEnemies(Actor* actor, GlobalContext* ctxt) {
    if (actor->id == ACTOR_EN_DG) {
        return;
    }

    if (actor->type != ACTORTYPE_ENEMY && actor->type != ACTORTYPE_BOSS) {
        return;
    }

    if (z2_Player_InBlockingCsMode(ctxt, GET_PLAYER(ctxt))) {
        return;
    }
    ...
    bool prevFrameAdvanceEnabled = ctxt->frameAdvCtx.enabled;
    ctxt->frameAdvCtx.enabled = true;
    actor->update(actor, ctxt);
    ctxt->frameAdvCtx.enabled = prevFrameAdvanceEnabled;
}

void Actor_Update(Actor* actor, GlobalContext* ctxt) {
    actor->update(actor, ctxt);

    if (MISC_CONFIG.flags.hyperEnemies) {
        Actor_HyperEnemies(actor, ctxt);
    }

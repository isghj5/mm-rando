...
void NpcKafei_KeatonMaskInit(ActorPlayer* actor) {
    ...
    if (MISC_CONFIG.MMRbytes.npcKafeiReplaceMask == 0) { //getItem isn't a mask, don't change from keaton
		return;
	}
	else
	{
		if (actor->mask != 0) { //still wearing mask, so draw something new
		...
		if (MISC_CONFIG.flags.freestanding) {
			if (MISC_CONFIG.MMRbytes.npcKafeiReplaceMask == 0) {
				...
				if (isHand) {
					...
					pos.y = 768; //412.15
					pos.z = -128; //-969.96
					rot.x = 0x0000; //0xCD42 -7 fb06
					rot.y = 0x0800; //0xAF22 -64 d20f
					rot.z = 0xE000; //0x88E9 90 4000
